...
#-----------------------------calendar-------news--------events-------weather----------
	...
	def addJobs( self ):
		...
		self.__scheduler.add_job( lambda: self.__socketServer.setWeatherFlag( True ), "interval", hours=3 )
		self.__scheduler.add_job( self.__news.getNews, "interval", hours=3 )#warning:not rendering
		self.__scheduler.add_job( lambda: self.__socketServer.setNewsFlag( True ), "interval", hours=3 )
